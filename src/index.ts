...
export const PROJECT: string = process.cwd();
 ...
 */
export function dirExists(...segments: string[]): boolean {
  try {
 ...
 */
export function listDir(dir: string, extension?: string): string[] {
 ...
 */
export function fileContains(relativePath: string, pattern: string | RegExp): boolean {
  if (!fs.existsSync(projectPath(relativePath))) return false;
 ...
 */
export function fileMatches(relativePath: string, pattern: RegExp): boolean {
 ...
 */
export function runCommand(command: string, timeout = 30000): string {
 ...
 */
export function expectBuildSucceeds(): void {
  runBuild();
 ...
 */
export async function importModule(relativePath: string): Promise<any> {
